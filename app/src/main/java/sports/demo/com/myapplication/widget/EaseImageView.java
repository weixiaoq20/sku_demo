...
public class EaseImageView extends ImageView {
    ...
    // radius
    private int radius;
    // rectangle or round, 1 is circle, 2 is rectangle,3 left rectangle
    ...
    private void drawDrawable(Canvas canvas, Bitmap bitmap) {
        ...
        if (shapeType == 1) {
            ...
            Path path = new Path();
            float[] radii = {radius + 1, radius + 1, 0f, 0f, 0f, 0f,  radius + 1,  radius + 1};
            path.addRoundRect(new RectF(0, 0,  getWidth() - 1, getHeight() - 1), radii, Path.Direction.CW);
